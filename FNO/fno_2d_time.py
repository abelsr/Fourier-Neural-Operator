"""
Implementation of Fourier Neural Operator for 2D Time-Dependent Problem using PyTorch Lightning.
...
class SpectralConv3D(L.LightningModule):
    """
    Spectral Convolution in Fourier Space (3D) [x, y, t]
    ...
    * modes: List[int] - Number of Fourier modes to multiply, default = 6 (2 modes per dimension) at most N/2 + 1
    ...
    def forward(self, x):
        ...
        out_ft = torch.zeros(batchsize, self.out_channels, x.size(-3), x.size(-2), x.size(-1)//2 + 1, dtype=torch.cfloat, device=self.device)
...
# Fourier Layer Block
class FourierLayerBlock(L.LightningModule):
    """
    Fourier Layer Block
    ...
    * layers: int - Number of layers in the block, default = 4
    ...
    def __init__(self, modes=(6, 6, 6), width=4):
        ...
        self.rama_superior = SpectralConv3D(self.width, self.width, self.modes).to(self.device)
        
        # 1D Convolution Layer
        self.rama_inferior = nn.Conv1d(self.width, self.width, 1).to(self.device)
...
class FNO2DTime(L.LightningModule):
    """
    Fourier Neural Operator 2D Time-Dependent
    ...
    * ti: int - Number of input features, default = 10
    ...
    def __init__(self, modes=(6, 6, 6), width=10, layers=4, ti=10, padding=0):
        ...
        * padding: int - Padding size, default = 0
        ...
        self.fc0 = nn.Linear(ti+3, self.width).to(self.device)
        
        # Array of Fourier Layer Blocks
        self.module = nn.ModuleList()
        for _ in range(self.layers):
            self.module.append(FourierLayerBlock(self.modes, self.width).to(self.device))
            
        # Linear Layers (Q)
        self.fc1 = nn.Linear(self.width, 128).to(self.device)
    ...
    def forward(self, x):
        ...
        if self.padding > 0:
            x = F.pad(x, (0, 0, 0, self.padding, self.padding, 0))
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.05)
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_size, x_size, y_size, t_size = y.shape
        out = self(x).view(batch_size, x_size, y_size, t_size)
        ...
        # Save the loss
        self.log('l2', l2, on_epoch=True, prog_bar=True, logger=True)
        self.log('mse', mse, on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Compute loss val
        mse = F.mse_loss(out.view(batch_size, -1), y.view(batch_size, -1), reduction='mean')
        l2 = self.loss_fn(out.view(batch_size, -1), y.view(batch_size, -1)).item()
        
        # Save the loss
        self.log('val_l2', l2, on_epoch=True, prog_bar=True, logger=True)
